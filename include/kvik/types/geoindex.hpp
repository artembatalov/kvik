...
public:
    ...
    std::optional<double> Dist(const std::string& m1, const std::string& m2) const {
        auto p1 = Pos(m1);
        auto p2 = Pos(m2);
    ...
    std::vector<SearchResult> Search(double lon, double lat, double radius_m, bool asc, int count) const {
        ...
        if (asc) {
            std::sort(results.begin(), results.end(), [](auto& a, auto& b) { return a.dist_m < b.dist_m; });
        } else {
            std::sort(results.begin(), results.end(), [](auto& a, auto& b) { return a.dist_m > b.dist_m; });
        }
